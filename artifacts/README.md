...
- `verify_datamodule.py` — end-to-end check of the region-grouped data pipeline: prints the
  manifest QC summary, the train/val region split (asserting the region sets are disjoint), and
  per-batch shapes/ranges/NaN counts; writes a train-vs-val sample grid.
...
# Produces: artifacts/output/samples.png  (requires data/subset/, see scripts/acquire_subset.py)
```
